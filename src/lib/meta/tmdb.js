...
export default class Tmdb {
  ...
  async getMovieById(id, language){
    try {
      ...
      if (meta) {
        return {
          ...
          year: parseInt(String(meta.release_date).split('-').shift()),
          imdb_id: id,
          type: 'movie',
          stremioId: id,
          id,
    ...
    const fallback = new Cinemeta();
    return fallback.getMovieById(id, language);
  ...
  async getEpisodeById(id, season, episode, language){
    try {
      ...
      const tvId = searchId.tv_results?.[0]?.id;
      if (tvId) {
        const meta = await this.#request('GET', '/3/tv/' + tvId, {query: {language: language || 'en-US'}}, {key: id + ':' + language, ttl: 3600*3});
        ...
        return {
          ...
          year: parseInt(String(meta.first_air_date).split('-').shift()),
          imdb_id: id,
          type: 'series',
    ...
    const fallback = new Cinemeta();
    return fallback.getEpisodeById(id, season, episode, language);
