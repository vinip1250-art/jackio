...
function tmdbHeaders() {
  const bearer = (process.env.TMDB_BEARER_TOKEN || process.env.TMDB_ACCESS_TOKEN || "").trim();
...
function hasTmdbAuth() {
  return !!((process.env.TMDB_API_KEY || "").trim() || (process.env.TMDB_BEARER_TOKEN || process.env.TMDB_ACCESS_TOKEN || "").trim());
}
