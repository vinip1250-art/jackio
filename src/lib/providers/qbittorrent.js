...
import { injectTrackers } from "../torrentEnrich.js";
...
const LOCAL_SAVE_DIR = !QBIT_SAVE_DIR || !DOCKER_PATH_MAPPING 
  ? QBIT_SAVE_DIR 
  : (QBIT_SAVE_DIR.startsWith(DOCKER_PATH_MAPPING.split(":")[0]) 
      ? path.normalize(QBIT_SAVE_DIR.replace(DOCKER_PATH_MAPPING.split(":")[0], DOCKER_PATH_MAPPING.split(":").slice(1).join(":"))) 
      : QBIT_SAVE_DIR);
...
if (!fs.existsSync(LOCAL_SAVE_DIR)) {
  try {
    ...
    console.warn(`[qBit] Aviso: Não foi possível criar o diretório QBIT_SAVE_DIR (${LOCAL_SAVE_DIR}): ${err.message}`);
  }
}
...
function resolveFilePath(info, file) {
  ...
  // 1ª tentativa: caminho direto em LOCAL_SAVE_DIR (ex: arquivo avulso)
  const byDir = path.join(LOCAL_SAVE_DIR, relative);
  const res1 = checkExists(byDir);
  if (res1) return res1;

  // 2ª tentativa: subpasta do hash — onde addTorrentBuffer/addMagnet salva o torrent
  if (info.hash) {
    const byHash = path.join(LOCAL_SAVE_DIR, info.hash.toLowerCase(), relative);
...
function getMimeType(filePath) {
  const cleanPath = filePath.replace(/\.!qB$/i, '');
